...
def icp(A, B, init_pose=None, max_iterations=20, tolerance=0.001, max_dist=1.2):
    '''
    ...
    for iteration in range(max_iterations):
        ...
        # remove outliers: process the output of nearest_neighbor (added by wuch)
        count = 0
        for distance in distances:
            if distance <= max_dist*2.0/(iteration + 1.0):
                count += 1

        src_good = np.zeros((m, count))  # good: outliers removed
