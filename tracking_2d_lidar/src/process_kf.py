...
class ProcessKF:
    ...
    def predict_update(self, laser_odom_now, clusters_now, rate):
        ...
        for i, track in enumerate(self.x.tracks):
            ...
            # static or dynamic (static map overlay check)
            track_size = track.xp.shape[1]
            ...
            # # add amcl covariance matrix to kf.R
            # track.kf.R = np.array([[0.1, 0.],
            #                        [0., 0.1]]) + \
            #             laser_odom_now.amcl_pose_cov * 100

            # kf predict and update
            track.kf.predict()
            track.kf.update(z)

            # static or dynamic (chi-squared test)
            # if track.static == False:  # only test those that do no t overlap with static map
            #     self.chi_squared_test(track)\
            # static or dynamic (speed threshold)
    ...
    def publish_to_costmap(self):       
        ...
        for track in self.x.tracks:
            if track.static == False and track.counter < 0:  # dynamic and mature
                msg = TracksMsg()
    ...
    def publish_kf_state(self):
        '''
        for checking estimation error
        '''
        i = 1  # user defined track id
